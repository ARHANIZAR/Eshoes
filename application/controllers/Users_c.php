...
class Users_c extends CI_Controller {
     ...
     *
     */
    public function validFormUpdate(){
        $this->form_validation->set_rules('login','login','trim|required');
        ...
        $donnees= array(
            'login'=>$this->input->post('login'),
            'email'=>$this->input->post('email'),
            'password'=>$this->input->post('pass')   // encryptage MD5 ou autre à faire
        );

        if($this->form_validation->run() == False){
            $this->load->view('head_v');
            $this->load->view('clients/navClient_v');
            $donnees['titre']="update";
            $this->load->view('clients/update_user_v',$donnees);
            $this->load->view('foot_v');
        }
        else{
            ...
            $donnees['droit']=1;
            $donnees['valide']=0;
            $this->Users_m->update_user($donnees);
            redirect(base_url());
        }
    }
    ...
    public function modifier(){
        if( $this->session->userdata('droit')==1) {
            $this->load->view('head_v');
            $this->load->view('clients/navClient_v');
            $data['user']=$this->Users_m->getUser();
            $this->load->view('clients/update_user_v',$data);
